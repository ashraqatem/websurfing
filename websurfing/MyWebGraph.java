...
public class MyWebGraph {
	...
	public boolean addVertex(String s) {
		//s is the the string representing the url
		// TODO : Add code here
	...
	public boolean addEdge(String s, String t) {
		//input takes two urls
		//an edge from the first url (s) to the 2nd url (t)
		// TODO : Add code here

		if (vertexList.containsKey(s) && vertexList.containsKey(t)) {
			WebVertex firstVer = vertexList.get(s);
			//firstVer is first url where the edge will start

			//when vertex is not connected to 2nd url connect them

			if (!firstVer.containsEdge(t)) {
				firstVer.addEdge(t);
				return true;
			}
		}
    ...
    public ArrayList<String> getVertices() {
		...
		//keySet will retireve all keys at once.
		//after making a set of these keys iterate through the set
		//and get each key into the listUrl that we will return
    ...
    public ArrayList<String> getEdgesInto(String v) {
    	// TODO : Add code here
		//an array we will add pages that have edges to v (input)
		ArrayList<String> listPages = new ArrayList<>();

		for (WebVertex vertex : vertexList.values()) {
			//if the vertex from the list of vertices has edge with v add it to the list of pages
			if (vertex.containsEdge(v)) {
				listPages.add(vertex.url);
			}
		}
		return listPages;
    }
